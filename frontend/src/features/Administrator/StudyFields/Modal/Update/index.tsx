...
import { updateStudyField } from "@src/store/slices/administrator/studyFields/thunk"
import { restoreUpdate } from "@src/store/slices/administrator/studyFields/slice"
...
const UpdateStudyField: React.FC<UpdateTableItemModalPropTypes> = ({ open, setOpen, item }) => {
  const dispatch = useAppDispatch()
  const formikRef = useRef<any>(null)

  const { status, error } = useAppSelector((state) => state.administrator.studyFields).update
  ...
  useEffect(() => {
    if (error) {
      message.error(`Échec lors de la modification de la filière : ${error}`)
      console.log("Error while updating study field:", error)
    }
  }, [error])

  useEffect(() => {
    if (status === "succeeded") {
      message.success("La filière a été modifiée avec succès.")
      setOpen(false)
    }
  }, [status, setOpen])
  ...
  return (
    <Modal open={open} title="Modifier une filière" closeModal={closeModal} destroyOnClose>
      <Formik
          ...
          return (
            <Form noValidate className="login-form flex flex-col w-full tiny:gap-2 sm:gap-4">
              <DiplomaAsyncSelect
                ...
                error={errors.diploma as string}
                touched={touched.diploma as boolean}
                defaultValue={item.diploma._id}
                placholder="Sélectionner le diplôme"
              />
              ...
              <Input
                ...
                setFieldTouched={setFieldTouched}
                setFieldValue={setFieldValue}
                placeholder="Saisissez le nom de la filière"
              ...
              <Input
                ...
                type="text"
                name="acronym"
                touched={touched.acronym as boolean}
                error={errors.acronym as string}
                setFieldTouched={setFieldTouched}
                setFieldValue={setFieldValue}
                placeholder="Saisissez l'acronyme de la filière"
                label="Acronyme"
...
export default UpdateStudyField
