...
import { deleteUnit } from "@src/store/slices/administrator/units/thunk"
import { restoreDelete } from "@src/store/slices/administrator/units/slice"
import Spinner from "@src/components/Spinner"

const DeleteUnit: React.FC<DeleteTableItemModalPropTypes> = ({ open, setOpen, item, page }) => {
  const dispatch = useAppDispatch()
  const { status, error } = useAppSelector((state) => state.administrator.units).delete
  ...
  useEffect(() => {
    if (status === "succeeded") {
      message.success("L'unité a été supprimée avec succès")
      setOpen(false)
    }
  }, [status, setOpen])

  const handleDelete = () => {
    dispatch(deleteUnit({ id: item.id, page }))
  }

  return (
    <Modal open={open} title="Supprimer l'unité" closeModal={() => setOpen(false)} destroyOnClose>
      <div className="flex flex-col items-center gap-8">
        <p className="text-center">
          Êtes-vous sûr de vouloir supprimer l'unité <b>{item.unit}</b> ?<br />
