...
import { deleteClass } from "@src/store/slices/administrator/classes/thunk"
import { restoreDelete } from "@src/store/slices/administrator/classes/slice"
import Spinner from "@src/components/Spinner"

const DeleteClass: React.FC<DeleteTableItemModalPropTypes> = ({ open, setOpen, item, page }) => {
  const dispatch = useAppDispatch()
  const { status, error } = useAppSelector((state) => state.administrator.classes).delete
  ...
  useEffect(() => {
    if (status === "succeeded") {
      message.success("La classe a été supprimée avec succès")
      setOpen(false)
    }
  }, [status, setOpen])

  const handleDelete = () => {
    dispatch(deleteClass({ id: item.id, page }))
  }

  return (
    <Modal open={open} title="Supprimer la classe" closeModal={() => setOpen(false)} destroyOnClose>
      <div className="flex flex-col items-center gap-8">
        <p className="text-center">
          Êtes-vous sûr de vouloir supprimer la classe <b>{item.classe}</b> ?<br />
