...
import { restoreDelete } from "@src/store/slices/administrator/users/slice"
import { deleteUser } from "@src/store/slices/administrator/users/thunk"

const DeleteUser: React.FC<DeleteTableItemModalPropTypes> = ({ open, page, item, setOpen }) => {
  const {
    delete: { status, error },
  ...
  useEffect(() => {
    if (error) {
      message.error(`Échec lors de la suppression de l'utilisateur. Veuillez réessayer.`)
  ...
  const handleSubmit = () => {
    dispatch(deleteUser({ id: item.id, page }))
  }

  return (
    <Modal open={open} title="Supprimer votre utilisateur" closeModal={closeModal} destroyOnClose>
