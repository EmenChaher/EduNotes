...
import CreateLevel from "./Modal/Create"
import DeleteLevel from "./Modal/Delete"
import UpdateLevel from "./Modal/Update"
...
const Levels: React.FC = () => {
  ...
  const columns: ColumnType<any>[] = [
    ...
    },
    {
      ...
      render: (studyField: any) => {
        if (!studyField) return "—";
        if (!studyField.label || !studyField.acronym || !studyField.diploma) return "—";
        if (!studyField.diploma.label) return `${studyField.label} (${studyField.acronym})`;
        return `${studyField.label} (${studyField.acronym}) - ${studyField.diploma.label}`;
      },
    },
    {
      title: "Action",
  ...
  const [currentPageLevels, setCurrentPageLevels] = useState<ILevel[]>([])
  ...
  const mapLevels = (docs: any[]) => {
    if (!docs || !Array.isArray(docs)) return [];
    
    return docs.map((level: any, index: number) => ({
      id: level._id,
      key: (index + 1 + (currentPage - 1) * levelsPerPage).toString(),
      ...level,
    }))
  }

  useEffect(() => {
    if (status === "succeeded" && levels && levels[currentPage]) {
      setCurrentPageLevels(levels[currentPage])
    }
  }, [levels, currentPage, status])

  const data = useMemo(() => mapLevels(currentPageLevels), [currentPageLevels, mapLevels])
  const totalData = useMemo(() => (levels !== null && !error ? total : 0), [total, levels, error])
  const dataLoading = status === "loading"
  const pageLoading = (levels === null && status !== "failed") || (dataLoading && total === null)

  useEffect(() => {
    if (!levels || !levels[currentPage]) {
      dispatch(restoreFetch())
      dispatch(fetchLevels({ page: currentPage, limit: levelsPerPage }))
    }
  }, [dispatch, levels, currentPage])
      ...
      {pageLoading ? (
        ...
        <>
          <Table
            ...
            createModal={CreateLevel}
            onChange={onChange}
          />
          {deleteSelectedItem && (
            <DeleteLevel open={deleteModalVisible} setOpen={setDeleteModalVisible} page={currentPage} item={deleteSelectedItem} />
          )}
          {updateSelectedItem && <UpdateLevel open={updateModalVisible} setOpen={setUpdateModalVisible} item={updateSelectedItem} />}
        </>
      )}
    </div>
  )
