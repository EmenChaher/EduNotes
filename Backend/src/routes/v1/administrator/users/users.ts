...
import { deleteUser, getAllUsers, promoteUser, updateUser } from '@controllers/administrator/users/users';
...
const getAllUsersSchema = Joi.object({
  ...
  cin: Joi.alternatives().try(Joi.string(), Joi.array().items(Joi.string())),
  name: Joi.alternatives().try(Joi.string(), Joi.array().items(Joi.string())),
  surname: Joi.alternatives().try(Joi.string(), Joi.array().items(Joi.string())),
  email: Joi.alternatives().try(Joi.string(), Joi.array().items(Joi.string())),
  telephone: Joi.alternatives().try(Joi.string(), Joi.array().items(Joi.string())),
  birthdate: Joi.alternatives().try(Joi.string(), Joi.array().items(Joi.string())),
  region: Joi.alternatives().try(Joi.string(), Joi.array().items(Joi.string())),
...
router.get('/users', validateSchema(getAllUsersSchema, ValidationSource.QUERY), getAllUsers);

router.patch(
  '/user/:id?',
  validateSchema(updateUserIdSchema, ValidationSource.PARAM),
  validateSchema(updateUserSchema, ValidationSource.BODY),
  updateUser,
);

router.delete('/user/:id?', validateSchema(deleteUserSchema, ValidationSource.PARAM), deleteUser);

router.post('/user/promote/:id?', validateSchema(promoteUserSchema, ValidationSource.PARAM), promoteUser);
